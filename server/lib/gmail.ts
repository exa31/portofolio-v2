...
function getOAuth2Client(): OAuth2Client {
    ...
    if (!clientId || !clientSecret) {
        throw new HttpError(500, 'GOOGLE_OAUTH_NOT_CONFIGURED', 'Google OAuth credentials not configured');
    }
...
export async function exchangeCodeForTokens(code: string, redirectUri: string = 'postmessage'): Promise<{
    email: string;
    ...
    if (!tokens.access_token) {
        throw new HttpError(400, 'GMAIL_AUTH_FAILED', 'Failed to get access token from Google');
    }
...
export async function sendEmail(
    ...
    if (!response.ok) {
        ...
        throw new HttpError(500, 'GMAIL_SEND_FAILED', `Failed to send email: ${response.status}`);
    }
}
...
export async function fetchAttachmentBuffer(fileUrl: string): Promise<Buffer> {
    ...
    if (!response.ok) {
        throw new HttpError(500, 'FETCH_ATTACHMENT_FAILED', `Failed to fetch attachment: ${response.status}`);
    }
